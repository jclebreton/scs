...
```
$ go get github.com/jclebreton/scs/v2
```

Note: If you're using the traditional `GOPATH` mechanism to manage dependencies, instead of modules, you'll need to `go get` and `import` `github.com/jclebreton/scs` without the `v2` suffix.

Please use [versioned releases](https://github.com/jclebreton/scs/releases). Code in tip may contain experimental features which are subject to change.
...
Documentation for all available settings and their default values can be [found here](https://godoc.org/github.com/jclebreton/scs#SessionManager).

### Working with Session Data

Data can be set using the [`Put()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.Put) method and retrieved with the [`Get()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.Get) method. A variety of helper methods like [`GetString()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.GetString), [`GetInt()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.GetInt) and [`GetBytes()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.GetBytes) are included for common data types. Please see [the documentation](https://godoc.org/github.com/jclebreton/scs#pkg-index) for a full list of helper methods.

The [`Pop()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.Pop) method (and accompanying helpers for common data types) act like a one-time `Get()`, retrieving the data and removing it from the session in one step. These are useful if you want to implement 'flash' message functionality in your application, where messages are displayed to the user once only.

Some other useful functions are [`Exists()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.Exists) (which returns a `bool` indicating whether or not a given key exists in the session data) and [`Keys()`](https://godoc.org/github.com/jclebreton/scs#SessionManager.Keys) (which returns a sorted slice of keys in the session data).
...
| Package                                                                               |                                                                                  |
|:------------------------------------------------------------------------------------- |----------------------------------------------------------------------------------|
| [badgerstore](https://github.com/jclebreton/scs/tree/master/badgerstore)       		| BadgerDB based session store  		                                               |
| [boltstore](https://github.com/jclebreton/scs/tree/master/boltstore)       			| BoltDB based session store  		                                               |
| [memstore](https://github.com/jclebreton/scs/tree/master/memstore)       			| In-memory session store (default)                                                |
| [mysqlstore](https://github.com/jclebreton/scs/tree/master/mysqlstore)   			| MySQL based session store                                                        |
| [postgresstore](https://github.com/jclebreton/scs/tree/master/postgresstore)         | PostgreSQL based session store                                                   |
| [redisstore](https://github.com/jclebreton/scs/tree/master/redisstore)       		| Redis based session store |
| [sqlite3store](https://github.com/jclebreton/scs/tree/master/sqlite3store) | SQLite3 based session store |
...
[`scs.Store`](https://godoc.org/github.com/jclebreton/scs#Store) defines the interface for custom session stores. Any object that implements this interface can be set as the store when configuring the session.
